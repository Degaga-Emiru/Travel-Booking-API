...
import { FiMapPin, FiStar, FiWifi, FiCoffee, FiTv, FiWind, FiCalendar, FiUsers, FiCheckCircle, FiArrowLeft } from 'react-icons/fi';
...
const HotelDetail = () => {
  ...
  const fromBookings = location.state?.fromBookings;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              <div className="flex items-center"><FiMapPin className="mr-1 text-primary-500" /> {hotel.city}, {hotel.country}</div>
              <div className="flex items-center"><FiStar className="mr-1 text-amber-500 fill-amber-500" /> {hotel.averageRating || hotel.rating || '4.5'} ({hotel.reviewCount || '0'} reviews)</div>
            </div>
          </motion.div>
          ...
          {hotel.Images && hotel.Images.length > 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4 h-[400px]">
              <img src={hotel.image || "https://images.unsplash.com/photo-1566073771259-6a8506099945"} className="w-full h-full object-cover rounded-3xl" alt={hotel.name} />
              <div className="grid grid-rows-2 gap-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {(hotel.amenities && hotel.amenities.length > 0 ? hotel.amenities : ['WiFi', 'Pool', 'Breakfast', 'Air Conditioning']).map((amenity, idx) => (
                <div key={idx} className="flex items-center p-4 bg-gray-50 rounded-2xl">
                  <FiCheckCircle className="mr-3 text-primary-500" /> {amenity}
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Right Column: Booking Card / Booking Info */}
        <div className="lg:col-span-1">
          {fromBookings ? (
            <div className="sticky top-24 bg-white p-8 rounded-3xl shadow-xl border border-gray-100 space-y-6">
              <h3 className="text-xl font-bold text-gray-900 border-b border-gray-100 pb-4">Booking Information</h3>
              <div className="space-y-4 text-sm">
                <div>
                  <span className="text-gray-400 block uppercase text-xs tracking-wider font-bold">Booking Status</span>
                ...
                <div>
                  <span className="text-gray-400 block uppercase text-xs tracking-wider font-bold">Booking Reference</span>
                  <span className="font-semibold text-gray-800">{booking?.bookingReference || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block uppercase text-xs tracking-wider font-bold">Check-in</span>
                  <span className="font-semibold text-gray-800">{booking?.checkInDate ? new Date(booking.checkInDate).toLocaleDateString() : 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block uppercase text-xs tracking-wider font-bold">Check-out</span>
                  <span className="font-semibold text-gray-800">{booking?.checkOutDate ? new Date(booking.checkOutDate).toLocaleDateString() : 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-400 block uppercase text-xs tracking-wider font-bold">Guests & Rooms</span>
                  <span className="font-semibold text-gray-800">{booking?.adults} Guests, {booking?.rooms || 1} Room(s)</span>
                </div>
                <div className="pt-4 border-t border-gray-100 flex justify-between items-center text-base">
                  <span className="text-gray-500 font-medium">Total Paid</span>
                  <span className="font-black text-gray-950 text-xl">${booking?.finalAmount}</span>
                </div>
              </div>
            </div>
          ) : (
